...
var KExport = class {
  ...
  async start() {
    ...
    workItems.sort((a, b) => a.title.length - b.title.length);
    ...
    for await (const mdfilePath of this.getFiles(articleSrcPath)) {
      ...
      if (item.title.toLowerCase().indexOf("(draft)") === -1 && item.tags.toLocaleLowerCase().indexOf("#unlisted") === -1) {
